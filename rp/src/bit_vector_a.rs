...
impl BitVectorA {
    ...
    pub fn rank_a(&self, c: u8, i: u64) -> Option<u64> {
    ...
    fn rank_one(&self, j: u64, i: u64) -> Option<u64> {
        self.rank_a(1, j + i*(self.t)).map( |x: u64| -> u64 { x - self.rank_a(1, i*self.t).unwrap() } )
    }

    fn select_one(&self, j: u64, i: u64) -> Option<u64> {
